...
class Test(GXPYTest):
    ...
    def test_local_dict(self):
        ...
        self.assertEqual('WGS 84 / *Local(43,-96,0,0)', csd.name)

        with gxcs.Coordinate_translate(csd, gxcs.Coordinate_system('WGS 84')) as pj:
            lon, lat, z = pj.convert((0, 0, 0))
            self.assertAlmostEqual(43, lat)
            self.assertAlmostEqual(-96, lon)
            self.assertAlmostEqual(0, z)

        csdict['azimuth'] = 25
        csd = gxcs.Coordinate_system(csdict)
        self.assertEqual('WGS 84 / *Local(43,-96,0,0) <0,0,0,0,0,25>', csd.name)
        self.assertEqual('"Oblique Stereographic",43,-96,0.9996,0,0', csd.gxf[2])
        ...
        with gxcs.Coordinate_translate(gxcs.Coordinate_system('WGS 84'), csd) as pj:
            ...
            self.assertAlmostEqual(73665.899715, x)
            self.assertAlmostEqual(34886.2319719, y)
            self.assertAlmostEqual(0, z)

        csdict['origin'] = (1800, 500)
        csd = gxcs.Coordinate_system(csdict)
        self.assertEqual('WGS 84 / *Local(43,-96,1800,500) <0,0,0,0,0,25>', csd.name)
        self.assertEqual('"Oblique Stereographic",43,-96,0.9996,1842.66314754,-307.558977615', csd.gxf[2])

        csdict['elevation'] = 800.5
        csdict['vcs'] = 'geoid'
        csd = gxcs.Coordinate_system(csdict)
        self.assertEqual('WGS 84 / *Local(43,-96,1800,500) <0,0,800.5,0,0,25>', csd.name)
        self.assertEqual('"Oblique Stereographic",43,-96,0.9996,1842.66314754,-307.558977615', csd.gxf[2])
        with gxcs.Coordinate_translate(gxcs.Coordinate_system('WGS 84'), csd) as pj:
            x, y = pj.convert((-96., 43.))
            self.assertAlmostEqual(1800, x)
            self.assertAlmostEqual(500, y)
        with gxcs.Coordinate_translate(csd, gxcs.Coordinate_system('WGS 84')) as pj:
            lon, lat, z = pj.convert((1800., 500., 0.))
            self.assertAlmostEqual(43, lat)
            self.assertAlmostEqual(-96, lon)
            self.assertAlmostEqual(800.5, z)
    ...
    def test_local(self):
        ...
        csd = gxcs.Coordinate_system.local(lon_lat=(-96, 43), azimuth=25, origin=(1800, 500),
                                           ...
                                           elevation=800.5, vcs='geoid')
        self.assertEqual('NAD83 / *Local(43,-96,1800,500) <0,0,800.5,0,0,25>', csd.name)
        self.assertEqual('"NAD83 to WGS 84 (1)",0,0,0,0,0,0,0', csd.gxf[4])
    ...
    def test_parameters(self):
        ...
        self.assertEqual(0.9996, float(params['P5']))
